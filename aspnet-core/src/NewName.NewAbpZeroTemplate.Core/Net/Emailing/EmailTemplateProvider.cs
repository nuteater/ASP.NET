...
namespace NewName.NewAbpZeroTemplate.Net.Emailing
{
    public class EmailTemplateProvider : IEmailTemplateProvider, ISingletonDependency
    {
        ...
        public string GetDefaultTemplate(int? tenantId)
        {
            ...
            {
                using (var stream = typeof(EmailTemplateProvider).GetAssembly().GetManifestResourceStream("NewName.NewAbpZeroTemplate.Net.Emailing.EmailTemplates.default.html"))
                {
