...
namespace NewName.NewAbpZeroTemplate.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : NewAbpZeroTemplateServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("NewName.NewAbpZeroTemplate", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
