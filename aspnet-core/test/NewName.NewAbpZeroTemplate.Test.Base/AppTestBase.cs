...
using NewName.NewAbpZeroTemplate.Test.Base.TestData;
...
namespace NewName.NewAbpZeroTemplate.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="NewAbpZeroTemplateDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(NewAbpZeroTemplateDbContext context)
            {
        ...
        protected void UsingDbContext(Action<NewAbpZeroTemplateDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<NewAbpZeroTemplateDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<NewAbpZeroTemplateDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<NewAbpZeroTemplateDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<NewAbpZeroTemplateDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<NewAbpZeroTemplateDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<NewAbpZeroTemplateDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<NewAbpZeroTemplateDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<NewAbpZeroTemplateDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<NewAbpZeroTemplateDbContext, Task<TResult>> func)
        {
            TResult result;
